...
class DatabaseConnector:
    ...
    def fetch_orders(self):
        query = "SELECT PELNY_NR_U_DOST FROM dbo.ZS_NIEOBSLUZONE"
        try:
            self.cursor.execute(query)
            rows = self.cursor.fetchall()
    ...
    def delete_order(self, order_no):
        query = "DELETE FROM dbo.ZS_NIEOBSLUZONE WHERE PELNY_NR_U_DOST = ?"
        try:
            self.cursor.execute(query, (order_no,))
            self.conn.commit()
            print(f"Order {order_no} deleted from ZS_NIEOBSLUZONE.")
        except pyodbc.Error as e:
            print(f"Error deleting order {order_no}: {e}")

    def fetch_invoices_correction(self):
